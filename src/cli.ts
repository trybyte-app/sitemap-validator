...
interface ProgressSnapshot {
  events: Partial<Record<ValidationEvent["type"], number>>;
  sourcesStarted: string[];
  sourcesFinished: string[];
  discoveredSources: SourceDiscovery[];
  bytesBySource: Record<string, number>;
  urlsBySource: Record<string, number>;
  sitemapsBySource: Record<string, number>;
}

interface CliOptionsReport {
  sitemapLocation: string | undefined;
  localSitemapRoot: string | undefined;
  publicUrlPrefix: string | undefined;
  childSitemapLoading: boolean;
  hreflangGraph: boolean;
  maxDepth: number;
  maxSources: number;
  loaderConcurrency: number;
...
interface CliPolicyReport {
  preset: CiPolicyPreset;
  failOn: readonly DiagnosticSeverity[];
  failOnRules: readonly string[];
  allowRules: readonly string[];
  maxWarnings: number | undefined;
}

interface StreamingPolicyState {
  failOn: ReadonlySet<DiagnosticSeverity>;
  failOnRules: ReadonlySet<string>;
  allowRules: ReadonlySet<string>;
  maxWarnings: number | undefined;
  warnings: number;
  errors: number;
...
interface CliReport {
  ...
  validatedAt: string;
  elapsedMs: number;
  options: CliOptionsReport;
  policy: CliPolicyReport;
...
async function validateFromCliArgs(args: CliArgs): Promise<CliReport> {
...
function writeUsage(output: WritableLike): void {
...
function createCliPolicy(args: CliArgs): CiPolicy {
...
function createPolicyReport(args: CliArgs, policy: CiPolicy): CliPolicyReport {
  return {
    preset: args.policy,
...
function createStreamingPolicyState(policy: CiPolicy): StreamingPolicyState {
  return {
...
function updatePolicyState(state: StreamingPolicyState, diagnostic: SitemapDiagnostic): void {
...
function finishPolicyEvaluation(state: StreamingPolicyState): CiEvaluation {
...
async function resolveLocalChildLoader(args: CliArgs): Promise<SitemapLoader | undefined> {
...
function updateProgress(progress: ProgressSnapshot, event: ValidationEvent): void {
...
function createFallbackSetSummary(summaries: readonly ValidationSummary[], counts: DiagnosticCounts): SitemapSetSummary {
  return {
...
function createJsonCliReport(report: CliReport, args: CliArgs): Omit<CliReport, "diagnostics"> & {
  diagnostics?: SitemapDiagnostic[];
...
function formatTextReport(report: CliReport, args: CliArgs): string {
  const lines = [
  ...
  lines.push(
    ...
    "Options",
    "-------",
    ...Object.entries(report.options).map(([key, value]) => `${key}: ${formatValue(value)}`),
    "",
    "Sources",
    "-------",
  );
  ...
  if (report.sourceSummaries.length === 0) {
    ...
    for (const summary of report.sourceSummaries) {
      lines.push(
        `${summary.sourceId}: root=${summary.rootType ?? "unknown"}, urls=${summary.urls}, sitemaps=${summary.sitemaps}, bytes=${summary.bytes}, diagnostics=${summary.diagnostics.errors}/${summary.diagnostics.warnings}/${summary.diagnostics.info}`,
      );
    }
  }
...
function formatDiagnosticLocation(diagnostic: SitemapDiagnostic): string {
  const parts = [
    diagnostic.sourceId,
