...
interface VideoEntry {
  ...
  tvShowShowTitle: string | undefined;
  tvShowVideoType: string | undefined;
  tvShowSeasonNumber: string | undefined;
  tvShowEpisodeNumber: string | undefined;
  tvShowPremierDate: string | undefined;
  tagCount: number;
}
...
function validateExtensionPlacement(
  state: State,
  element: Pick<StackItem, "local" | "uri" | "path">,
  parser: XmlParserAdapter,
): void {
...
function isValidVideoPlacement(local: string, parent: StackItem): boolean {
...
function isValidPageMapPlacement(local: string, parent: StackItem, grandparent: StackItem | undefined): boolean {
...
function isValidNewsPlacement(local: string, parent: StackItem, grandparent: StackItem | undefined): boolean {
  if (local === "news") {
...
function validateExtensionChildOrder(
  state: State,
  entry: { lastChildOrder: number },
  orderMap: ReadonlyMap<string, number>,
  code: string,
  element: Pick<StackItem, "local" | "path">,
  parser: XmlParserAdapter,
...
function isAllowedVideoAttribute(elementLocal: string, attributeLocal: string): boolean {
  if (elementLocal === "player_loc") return attributeLocal === "allow_embed" || attributeLocal === "autoplay";
  if (elementLocal === "restriction") return attributeLocal === "relationship";
  if (elementLocal === "platform") return attributeLocal === "relationship";
  if (elementLocal === "uploader") return attributeLocal === "info";
  if (elementLocal === "gallery_loc") return attributeLocal === "title";
  if (elementLocal === "content_segment_loc") return attributeLocal === "duration";
  if (elementLocal === "id") return attributeLocal === "type";
  if (elementLocal === "price") return attributeLocal === "currency" || attributeLocal === "type" || attributeLocal === "resolution";
  return false;
}

function isAllowedPageMapAttribute(elementLocal: string, attributeLocal: string): boolean {
  if (elementLocal === "Template") return attributeLocal === "src";
  if (elementLocal === "DataObject") return attributeLocal === "type" || attributeLocal === "id";
  if (elementLocal === "Attribute") return attributeLocal === "name" || attributeLocal === "value";
  return false;
}
...
function isAllowedHreflangAttribute(attributeLocal: string): boolean {
...
function validateImageChildCardinality(
  state: State,
  element: Pick<StackItem, "local" | "path">,
  parser: XmlParserAdapter,
): void {
...
function validateNewsChildCardinality(
  state: State,
  element: Pick<StackItem, "local" | "path">,
  parser: XmlParserAdapter,
): void {
  ...
  if (parent.uri === NEWS_NS && parent.local === "publication" && (element.local === "name" || element.local === "language")) {
    validateExtensionChildOrder(
      state,
      {
        get lastChildOrder() {
          return news.lastPublicationChildOrder;
        },
        set lastChildOrder(value: number) {
          news.lastPublicationChildOrder = value;
        },
      },
      NEWS_PUBLICATION_CHILD_ORDER,
      "GOOGLE_NEWS_ELEMENT_OUT_OF_ORDER",
      element,
      parser,
    );
...
function validateVideoChildCardinality(
  state: State,
  element: Pick<StackItem, "local" | "path">,
  node: XmlElement,
  parser: XmlParserAdapter,
...
function validatePageMapChildCardinality(
  state: State,
  element: Pick<StackItem, "local" | "path">,
  parser: XmlParserAdapter,
): void {
...
function validatePageMapRequiredAttributes(
  state: State,
  element: Pick<StackItem, "local" | "path">,
  node: XmlElement,
  parser: XmlParserAdapter,
...
function requiredPageMapAttribute(elementLocal: string): string | undefined {
  if (elementLocal === "Template") return "src";
  if (elementLocal === "DataObject") return "type";
  if (elementLocal === "Attribute") return "name";
  return undefined;
}

function validateVideoContentSegmentAttributes(state: State, node: XmlElement, path: string, parser: XmlParserAdapter): void {
...
function validateVideoIdAttributes(state: State, node: XmlElement, path: string, parser: XmlParserAdapter): void {
...
function handleCloseElement(state: State, item: StackItem, parser: XmlParserAdapter): void {
  ...
  if (state.currentNews && item.uri === NEWS_NS) {
    if (item.local === "name") state.currentNews.publicationName = text;
    if (item.local === "language") state.currentNews.publicationLanguage = text;
    if (item.local === "access") state.currentNews.access = text;
    if (item.local === "genres") state.currentNews.genres = text;
    if (item.local === "publication_date") state.currentNews.publicationDate = text;
    if (item.local === "title") state.currentNews.title = text;
    if (item.local === "keywords") state.currentNews.keywords = text;
    if (item.local === "stock_tickers") state.currentNews.stockTickers = text;
  }

  if (state.currentVideo && item.uri === VIDEO_NS) {
    if (item.local === "thumbnail_loc") state.currentVideo.thumbnailLoc = text;
    if (item.local === "title") state.currentVideo.title = text;
    if (item.local === "description") state.currentVideo.description = text;
    if (item.local === "content_loc") state.currentVideo.contentLoc = text;
    if (item.local === "player_loc") state.currentVideo.playerLoc = text;
    ...
    if (item.local === "duration") state.currentVideo.duration = text;
    if (item.local === "expiration_date") state.currentVideo.expirationDate = text;
    if (item.local === "publication_date") state.currentVideo.publicationDate = text;
    if (item.local === "rating") state.currentVideo.rating = text;
    if (item.local === "view_count") state.currentVideo.viewCount = text;
    if (item.local === "family_friendly") state.currentVideo.familyFriendly = text;
    if (item.local === "live") state.currentVideo.live = text;
    if (item.local === "requires_subscription") state.currentVideo.requiresSubscription = text;
    if (item.local === "restriction") state.currentVideo.restrictionValue = text;
    if (item.local === "gallery_loc") state.currentVideo.galleryLoc = text;
    if (item.local === "category") state.currentVideo.category = text;
    if (item.local === "platform") state.currentVideo.platformValue = text;
    if (item.local === "uploader") state.currentVideo.uploader = text;
    if (item.local === "tag") state.currentVideo.tagCount += 1;
    if (item.local === "show_title") state.currentVideo.tvShowShowTitle = text;
    if (item.local === "video_type") state.currentVideo.tvShowVideoType = text;
    if (item.local === "season_number") state.currentVideo.tvShowSeasonNumber = text;
    if (item.local === "episode_number") state.currentVideo.tvShowEpisodeNumber = text;
    if (item.local === "premier_date") state.currentVideo.tvShowPremierDate = text;
...
function validateGoogleDateTime(
  state: State,
  value: string,
  path: string,
  parser: XmlParserAdapter,
...
interface DateTimeValidationOptions {
  requireTimeSeconds?: boolean | undefined;
}

function isValidCompleteW3cDateOrDateTime(value: string, options: DateTimeValidationOptions = {}): boolean {
...
function validateHreflangs(state: State, entry: UrlEntry, path: string, parser: XmlParserAdapter): void {
...
function toPublicHreflangs(entries: readonly HreflangEntry[]): HreflangAlternate[] | undefined {
  const alternates = entries
    .filter((entry): entry is HreflangAlternate & Pick<HreflangEntry, "path"> => typeof entry.hreflang === "string" && typeof entry.href === "string")
    .map((entry) => ({
...
function validateNewsEntry(state: State, entry: NewsEntry, parser: XmlParserAdapter): void {
  const required: Array<[keyof NewsEntry, string]> = [
  ...
  if (entry.publicationDate !== undefined) {
    validateGoogleDateTime(
      state,
      entry.publicationDate,
      `${entry.path}/news:publication_date`,
      parser,
      "GOOGLE_NEWS_PUBLICATION_DATE_INVALID",
      "news:publication_date must use W3C date or datetime format.",
...
function validateVideoEntry(state: State, entry: VideoEntry, parser: XmlParserAdapter): void {
  const required: Array<[keyof VideoEntry, string]> = [
  ...
  if (entry.expirationDate !== undefined) {
    validateGoogleDateTime(
      ...
      `${entry.path}/video:expiration_date`,
      parser,
      "GOOGLE_VIDEO_EXPIRATION_DATE_INVALID",
      "video:expiration_date must use W3C date or datetime format; datetime values must include seconds and a timezone.",
      "https://developers.google.com/search/docs/crawling-indexing/sitemaps/video-sitemaps",
      { requireTimeSeconds: true },
    );
  }
  ...
  if (entry.publicationDate !== undefined) {
    validateGoogleDateTime(
      state,
      entry.publicationDate,
      `${entry.path}/video:publication_date`,
      parser,
      "GOOGLE_VIDEO_PUBLICATION_DATE_INVALID",
      "video:publication_date must use W3C date or datetime format; datetime values must include seconds and a timezone.",
      "https://developers.google.com/search/docs/crawling-indexing/sitemaps/video-sitemaps",
      { requireTimeSeconds: true },
    );
  }
...
function validateVideoPriceEntry(state: State, entry: VideoPriceEntry, parser: XmlParserAdapter): void {
...
function validatePageMapAttributeEntry(
  state: State,
  entry: PageMapAttributeEntry,
  text: string,
  parser: XmlParserAdapter,
): void {
...
function validateVideoTvShow(state: State, entry: VideoEntry, parser: XmlParserAdapter): void {
  if (!entry.tvShowPath) {
    return;
  }
  ...
  if (entry.tvShowPremierDate !== undefined) {
    validateGoogleDateTime(
      state,
      entry.tvShowPremierDate,
      `${entry.tvShowPath}/video:premier_date`,
      parser,
      "GOOGLE_VIDEO_TVSHOW_PREMIER_DATE_INVALID",
      "video:tvshow/video:premier_date must use W3C date or datetime format; datetime values must include seconds and a timezone.",
...
function validateHreflangLink(state: State, node: XmlElement, parser: XmlParserAdapter): void {
...
function isExtensionNamespace(uri: string): boolean {
