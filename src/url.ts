...
export interface UrlValidationResult {
  ...
  diagnostics: Array<Omit<SitemapDiagnostic, "sourceId" | "location"> & { layer: UrlValidationLayer }>;
}
...
export function validateSitemapUrlValue(
  value: string,
  source: "sitemaps.org" | "google",
  options: UrlValidationOptions = {},
): UrlValidationResult {
  const diagnostics: UrlValidationResult["diagnostics"] = [];
...
function toAsciiHostname(hostname: string): string | undefined {
  try {
...
function addPercentEncodingDiagnostics(value: string, diagnostics: UrlValidationResult["diagnostics"]): void {
...
function addUnsafeCharacterDiagnostics(value: string, diagnostics: UrlValidationResult["diagnostics"]): void {
  if (/[\u0020"<>\\^`{|}]/u.test(value)) {
    diagnostics.push({
...
function addHostDiagnostics(url: URL, diagnostics: UrlValidationResult["diagnostics"]): void {
...
function hasValidPercentEncodedUtf8Runs(value: string): boolean {
