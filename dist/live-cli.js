...
const AUDIT_FAIL_CHOICES = ["none", "error", "warning"];
const STATUS_METHOD_CHOICES = ["head", "get"];
...
function createLiveProgressLogger(args, io) {
    if (args.quiet) {
        return {
            info() { },
        };
...
export function parseLiveCliArgs(argv) {
...
function createLiveAuditReport(urlDataset, enabledChecks, uniqueUrls, auditedUrls, args, urlSource, accumulator) {
    return {
        enabledChecks,
        totalUrls: urlDataset.totalUrls,
        uniqueUrls,
...
async function collectAuditUrlSample(urlDataset, maxAuditUrls) {
    const records = [];
    const seen = new Set();
...
async function* urlRecordsFromArray(records) {
...
function auditCanonical(record, headers, html, requireCanonical, finalUrl, redirects) {
    const url = record.url;
    const canonicalUrls = [
...
function metadataAuditContext(record, finalUrl, redirects) {
    return redirects.length > 0
        ? { ...urlRecordContext(record), finalUrl, redirects }
        : urlRecordContext(record);
}
function extractCanonicalFromLinkHeader(value) {
    if (!value) {
        return [];
    }
    const canonicals = [];
...
function createLiveEvaluation(xmlEvaluation, audits, auditFailOn) {
...
function isAuditPassed(audits, auditFailOn) {
    if (auditFailOn === "none") {
        return true;
    }
...
function auditFailureReasons(audits, auditFailOn) {
    if (auditFailOn === "none") {
        return [];
    }
...
function formatJsonLiveReport(report, args) {
    if (args.detail === "full") {
        return report;
    }
...
function formatTextLiveReport(report) {
...
function createSkippedXmlReport() {
    return {
...
function getEnabledChecks(args) {
    const checks = [];
    if (args.checkDuplicates)
    ...
    if (args.checkStatus)
        checks.push("status");
    if (args.checkCanonical || args.requireCanonical)
        checks.push(args.requireCanonical ? "canonical-required" : "canonical");
    if (args.checkNoindex)
        checks.push("noindex");
    return checks;
}
function shouldLogMilestone(count, interval) {
    return count > 0 && count % interval === 0;
}
function shouldLogEarlyOrInterval(count, interval) {
    return count > 0 && (count <= 5 || count % interval === 0);
}
function logAuditUrlProgress(logger, label, count, previousCount) {
...
function invalidAuditUrlFinding(input) {
...
function writeLiveUsage(output) {
