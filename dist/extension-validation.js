...
export class ExtensionValidator {
    ...
    isTopLevelUrlExtension(element, parent) {
        return this.#urlActive
            && parent?.uri === SITEMAP_NS
            && parent.local === "url"
            && isExtensionNamespace(element.uri);
    }
    validatePlacement(element, parent, grandparent, context) {
        if (!isExtensionNamespace(element.uri))
            return;
        if (!this.#urlActive) {
            this.#add(context, "EXTENSION_OUTSIDE_URL", "Sitemap extension elements must be nested inside a url entry.", element.path, "https://developers.google.com/search/docs/crawling-indexing/sitemaps/combine-sitemap-extensions");
            return;
        }
        if (!parent)
            return;
        ...
        if (!valid)
            this.#add(context, code, message, element.path, spec);
    }
    open(element, node, parent, context) {
        if (!isExtensionNamespace(element.uri))
            return;
        this.#validateAttributes(element, node, context);
        if (element.uri === IMAGE_NS)
            this.#openImage(element, parent, context);
        if (element.uri === NEWS_NS)
            this.#openNews(element, parent, context);
        if (element.uri === VIDEO_NS)
            this.#openVideo(element, node, parent, context);
        if (element.uri === XHTML_NS && element.local === "link" && this.#urlActive)
            this.#openHreflang(element, node, context);
        if (element.uri === PAGEMAP_NS)
            this.#openPageMap(element, node, parent, context);
    }
    close(element, text, parentUrlLoc, context) {
        if (element.uri === IMAGE_NS)
            this.#closeImage(element, text, context);
        if (element.uri === NEWS_NS)
            this.#closeNews(element, text, context);
        if (element.uri === VIDEO_NS)
            this.#closeVideo(element, text, parentUrlLoc, context);
        if (element.uri === PAGEMAP_NS)
            this.#closePageMap(element, text, context);
    }
    #openImage(element, parent, context) {
        ...
        if (!image || element.local === "image" || parent?.uri !== IMAGE_NS || parent.local !== "image" || !KNOWN_IMAGE_TAGS.has(element.local))
            return;
    ...
    #closeImage(element, text, context) {
    ...
    #openNews(element, parent, context) {
        if (element.local === "news" && this.#urlActive) {
            ...
            if (this.#newsCount > 1)
                this.#add(context, "GOOGLE_NEWS_ENTRY_DUPLICATE", "Each url entry can contain only one news:news element.", element.path, NEWS_SPEC);
        ...
        if (!news || element.local === "news" || !parent)
            return;
        if (parent.uri === NEWS_NS && parent.local === "news") {
            ...
            if (NEWS_CHILD_ORDER.has(element.local)) {
                if (news.seenChildren.has(element.local))
                    this.#add(context, "GOOGLE_NEWS_ELEMENT_DUPLICATE", `news:${element.local} can appear only once in a news:news entry.`, element.path, NEWS_SPEC);
        ...
        if (parent.uri === NEWS_NS && parent.local === "publication" && (element.local === "name" || element.local === "language")) {
            const holder = {
                ...
                set lastChildOrder(value) { news.lastPublicationChildOrder = value; },
            };
            ...
            if (news.seenPublicationChildren.has(element.local))
                this.#add(context, "GOOGLE_NEWS_ELEMENT_DUPLICATE", `news:${element.local} can appear only once in news:publication.`, element.path, NEWS_SPEC);
            news.seenPublicationChildren.add(element.local);
        }
    }
    #closeNews(element, text, context) {
        const news = this.#currentNews;
        if (news) {
            if (element.local === "name")
                news.publicationName = text;
            if (element.local === "language")
                news.publicationLanguage = text;
            if (element.local === "access")
                news.access = text;
            if (element.local === "genres")
                news.genres = text;
            if (element.local === "publication_date")
                news.publicationDate = text;
            if (element.local === "title")
                news.title = text;
            if (element.local === "keywords")
                news.keywords = text;
            if (element.local === "stock_tickers")
                news.stockTickers = text;
        }
    ...
    #validateNews(entry, context) {
        for (const [value, label] of [
            ...
            if (!value)
                this.#add(context, "GOOGLE_NEWS_REQUIRED_FIELD", `news:news must contain ${label}.`, entry.path, NEWS_SPEC);
        }
        ...
        if (entry.genres !== undefined && !isValidGoogleNewsGenres(entry.genres))
            this.#add(context, "GOOGLE_NEWS_GENRES_INVALID", "news:genres must contain comma-separated values from the Google News XSD genre list.", `${entry.path}/news:genres`, NEWS_XSD);
        if (entry.stockTickers !== undefined && !isValidGoogleNewsStockTickers(entry.stockTickers))
            this.#add(context, "GOOGLE_NEWS_STOCK_TICKERS_INVALID", "news:stock_tickers must contain up to five comma-separated exchange:ticker values.", `${entry.path}/news:stock_tickers`, NEWS_XSD);
    }
    #openVideo(element, node, parent, context) {
        ...
        if (DEPRECATED_VIDEO_TAGS.has(element.local))
            this.#add(context, "GOOGLE_VIDEO_TAG_DEPRECATED", `video:${element.local} has been removed from Google's video sitemap documentation.`, element.path, VIDEO_SPEC, "warning");
        if (!KNOWN_VIDEO_TAGS.has(element.local))
            this.#add(context, "GOOGLE_VIDEO_UNKNOWN_TAG", `video:${element.local} is not a recognized Google video sitemap tag.`, element.path, VIDEO_SPEC, "warning");
        const video = this.#currentVideo;
        if (!video || element.local === "video")
            return;
        if (parent?.uri === VIDEO_NS && parent.local === "video" && KNOWN_VIDEO_TAGS.has(element.local)) {
            ...
            if (!VIDEO_REPEATABLE_TAGS.has(element.local)) {
                if (video.seenChildren.has(element.local))
                    this.#add(context, "GOOGLE_VIDEO_ELEMENT_DUPLICATE", `video:${element.local} can appear only once in a video:video entry.`, element.path, VIDEO_SPEC);
            ...
            if (element.local === "player_loc") {
                for (const attribute of Object.values(node.attributes)) {
                    if (DEPRECATED_VIDEO_PLAYER_LOC_ATTRIBUTES.has(attribute.local))
                        this.#add(context, "GOOGLE_VIDEO_TAG_DEPRECATED", `video:player_loc @${attribute.local} has been removed from Google's video sitemap documentation.`, `${element.path}/@${attribute.local}`, VIDEO_SPEC, "warning");
                }
                const allowEmbed = getAttribute(node.attributes, "allow_embed");
                if (allowEmbed !== undefined && !isVideoYesNo(allowEmbed))
                    this.#add(context, "GOOGLE_VIDEO_PLAYER_ALLOW_EMBED_INVALID", "video:player_loc @allow_embed must be yes or no, using a case variant allowed by the video sitemap XSD.", `${element.path}/@allow_embed`, VIDEO_XSD);
            }
        }
        if (element.local === "restriction")
            video.restrictionRelationship = getAttribute(node.attributes, "relationship");
        if (element.local === "platform")
            video.platformRelationship = getAttribute(node.attributes, "relationship");
        if (element.local === "uploader")
            video.uploaderInfo = getAttribute(node.attributes, "info");
        if (element.local === "content_segment_loc") {
            video.contentSegmentLocCount += 1;
            const duration = getAttribute(node.attributes, "duration");
            if (duration !== undefined && (!/^\d+$/.test(duration) || Number(duration) > 28_800))
                this.#add(context, "GOOGLE_VIDEO_CONTENT_SEGMENT_DURATION_INVALID", "video:content_segment_loc @duration must be a non-negative integer no greater than 28800.", `${element.path}/@duration`, VIDEO_XSD);
        }
        if (element.local === "id") {
            const type = getAttribute(node.attributes, "type");
            if (!type || !VIDEO_ID_TYPE_VALUES.has(type))
                this.#add(context, "GOOGLE_VIDEO_ID_TYPE_INVALID", "video:id must include a type attribute with one of the values defined by the video sitemap XSD.", `${element.path}/@type`, VIDEO_XSD);
        }
        ...
        if (element.local === "tvshow")
            video.tvShowPath = element.path;
    }
    #closeVideo(element, text, parentUrlLoc, context) {
        const video = this.#currentVideo;
        if (video) {
            const fields = {
            ...
            if (field)
                video[field] = text;
            if (element.local === "tag")
                video.tagCount += 1;
            if (element.local === "content_segment_loc")
                context.validateLoc(text, element.path, { enforceSitemapLocation: false, allowedProtocols: VIDEO_ALLOWED_PROTOCOLS });
    ...
    #validateVideo(entry, parentUrlLoc, context) {
        for (const [value, label] of [[entry.thumbnailLoc, "video:thumbnail_loc"], [entry.title, "video:title"], [entry.description, "video:description"]]) {
            if (!value)
                this.#add(context, "GOOGLE_VIDEO_REQUIRED_FIELD", `video:video must contain ${label}.`, entry.path, VIDEO_SPEC);
        }
        if (!entry.contentLoc && !entry.playerLoc)
            this.#add(context, "GOOGLE_VIDEO_LOCATION_REQUIRED", "video:video must contain either video:content_loc or video:player_loc.", entry.path, VIDEO_SPEC);
        if (entry.contentLoc !== undefined && entry.contentLoc === parentUrlLoc)
            this.#add(context, "GOOGLE_VIDEO_CONTENT_LOC_EQUALS_PAGE_LOC", "video:content_loc must not be the same URL as the parent page loc.", `${entry.path}/video:content_loc`, VIDEO_SPEC);
        if (entry.contentLoc !== undefined && hasUnsupportedVideoContentLocFormat(entry.contentLoc))
            this.#add(context, "GOOGLE_VIDEO_CONTENT_LOC_FORMAT_UNSUPPORTED", "video:content_loc should point directly to a supported video file, not an HTML page or Flash file.", `${entry.path}/video:content_loc`, VIDEO_SPEC);
        ...
        if (entry.viewCount !== undefined && (!/^\d+$/.test(entry.viewCount) || Number(entry.viewCount) < 0))
            this.#add(context, "GOOGLE_VIDEO_VIEW_COUNT_INVALID", "video:view_count must be a non-negative integer.", `${entry.path}/video:view_count`, VIDEO_SPEC);
        if (entry.familyFriendly !== undefined && !isVideoYesNo(entry.familyFriendly))
            this.#add(context, "GOOGLE_VIDEO_FAMILY_FRIENDLY_INVALID", "video:family_friendly must be yes or no, using a case variant allowed by the video sitemap XSD.", `${entry.path}/video:family_friendly`, VIDEO_SPEC);
        if (entry.live !== undefined && !isVideoYesNo(entry.live))
            this.#add(context, "GOOGLE_VIDEO_LIVE_INVALID", "video:live must be yes or no, using a case variant allowed by the video sitemap XSD.", `${entry.path}/video:live`, VIDEO_SPEC);
        if (entry.requiresSubscription !== undefined && !isVideoYesNo(entry.requiresSubscription))
            this.#add(context, "GOOGLE_VIDEO_REQUIRES_SUBSCRIPTION_INVALID", "video:requires_subscription must be yes or no, using a case variant allowed by the video sitemap XSD.", `${entry.path}/video:requires_subscription`, VIDEO_SPEC);
        ...
        if (entry.restrictionValue !== undefined && !isSpaceSeparatedIso3166List(entry.restrictionValue))
            this.#add(context, "GOOGLE_VIDEO_RESTRICTION_COUNTRY_INVALID", "video:restriction must contain a space-delimited list of ISO 3166 alpha-2 country codes.", `${entry.path}/video:restriction`, VIDEO_SPEC);
        ...
        if (entry.uploaderInfo !== undefined) {
            ...
            if (parentUrlLoc && !hasSameHostname(entry.uploaderInfo, parentUrlLoc))
                this.#add(context, "GOOGLE_VIDEO_UPLOADER_INFO_DOMAIN_INVALID", "video:uploader info must be on the same domain as the parent page loc.", `${entry.path}/video:uploader/@info`, VIDEO_SPEC);
        }
        if (entry.tagCount > 32)
            this.#add(context, "GOOGLE_VIDEO_TAG_LIMIT_EXCEEDED", "video:video must not contain more than 32 video:tag elements.", `${entry.path}/video:tag`, VIDEO_SPEC);
        this.#validateVideoTvShow(entry, context);
        if (entry.expirationDate !== undefined)
            this.#validateDate(context, entry.expirationDate, `${entry.path}/video:expiration_date`, "GOOGLE_VIDEO_EXPIRATION_DATE_INVALID", "video:expiration_date must use W3C date or datetime format; datetime values must include seconds and a timezone.", VIDEO_SPEC, true);
        if (entry.publicationDate !== undefined)
            this.#validateDate(context, entry.publicationDate, `${entry.path}/video:publication_date`, "GOOGLE_VIDEO_PUBLICATION_DATE_INVALID", "video:publication_date must use W3C date or datetime format; datetime values must include seconds and a timezone.", VIDEO_SPEC, true);
        for (const [value, label] of [[entry.thumbnailLoc, "thumbnail_loc"], [entry.contentLoc, "content_loc"], [entry.playerLoc, "player_loc"], [entry.galleryLoc, "gallery_loc"]]) {
            if (value !== undefined)
                context.validateLoc(value, `${entry.path}/video:${label}`, { enforceSitemapLocation: false, allowedProtocols: VIDEO_ALLOWED_PROTOCOLS });
        }
    }
    #validateRelationship(relationship, value, label, code, context, path) {
        if (!relationship && value !== undefined)
            this.#add(context, code, `video:${label} must include a relationship attribute.`, `${path}/video:${label}/@relationship`, VIDEO_SPEC);
        if (relationship && relationship !== "allow" && relationship !== "deny")
            this.#add(context, code, `video:${label} relationship must be allow or deny.`, `${path}/video:${label}/@relationship`, VIDEO_SPEC);
    }
    #validateVideoPrice(entry, context) {
    ...
    #validateVideoTvShow(entry, context) {
        if (!entry.tvShowPath)
            return;
        if (!entry.tvShowShowTitle)
            this.#add(context, "GOOGLE_VIDEO_TVSHOW_REQUIRED_FIELD", "video:tvshow must contain video:show_title.", entry.tvShowPath, VIDEO_XSD);
        if (!entry.tvShowVideoType)
            this.#add(context, "GOOGLE_VIDEO_TVSHOW_REQUIRED_FIELD", "video:tvshow must contain video:video_type.", entry.tvShowPath, VIDEO_XSD);
        else if (!VIDEO_TVSHOW_VIDEO_TYPE_VALUES.has(entry.tvShowVideoType))
            this.#add(context, "GOOGLE_VIDEO_TVSHOW_VIDEO_TYPE_INVALID", "video:tvshow/video_type must be one of full, preview, clip, interview, news, or other.", `${entry.tvShowPath}/video:video_type`, VIDEO_XSD);
        for (const [value, label] of [[entry.tvShowSeasonNumber, "season_number"], [entry.tvShowEpisodeNumber, "episode_number"]]) {
            if (value !== undefined && (!/^\d+$/.test(value) || Number(value) < 1))
                this.#add(context, "GOOGLE_VIDEO_TVSHOW_NUMBER_INVALID", `video:tvshow/${label} must be an integer greater than or equal to 1.`, `${entry.tvShowPath}/video:${label}`, VIDEO_XSD);
        }
        if (entry.tvShowPremierDate !== undefined)
            this.#validateDate(context, entry.tvShowPremierDate, `${entry.tvShowPath}/video:premier_date`, "GOOGLE_VIDEO_TVSHOW_PREMIER_DATE_INVALID", "video:tvshow/video:premier_date must use W3C date or datetime format; datetime values must include seconds and a timezone.", VIDEO_XSD, true);
    }
    #openHreflang(element, node, context) {
        ...
        if (rel !== "alternate")
            this.#add(context, "GOOGLE_HREFLANG_REL_INVALID", "xhtml:link hreflang annotations must use rel=\"alternate\".", element.path, HREFLANG_SPEC);
        if (!hreflang)
            this.#add(context, "GOOGLE_HREFLANG_REQUIRED", "xhtml:link alternate annotations must include hreflang.", element.path, HREFLANG_SPEC);
        else if (!isValidBcp47LanguageTag(hreflang))
            this.#add(context, "GOOGLE_HREFLANG_INVALID", "hreflang must be x-default or a valid-looking language/region code.", element.path, HREFLANG_SPEC);
        else if (!isGoogleSupportedHreflangTag(hreflang))
            this.#add(context, "GOOGLE_HREFLANG_UNSUPPORTED_CODE", "hreflang is valid BCP 47, but Google sitemap hreflang supports a two-letter language code with optional script and two-letter region, or x-default.", element.path, HREFLANG_SPEC);
        if (!href)
            this.#add(context, "GOOGLE_HREFLANG_HREF_REQUIRED", "xhtml:link alternate annotations must include href.", element.path, HREFLANG_SPEC);
        else
            context.validateLoc(href, `${element.path}/@href`, { enforceSitemapLocation: false });
        this.#hreflangs.push({ hreflang, href, path: element.path });
    }
    #validateHreflangs(loc, path, context) {
        if (this.#hreflangs.length === 0)
            return undefined;
        const seen = new Set();
        let hasSelfReference = false;
        const locKey = loc ? normalizeUrlKey(loc) : undefined;
        for (const entry of this.#hreflangs) {
            if (entry.href && locKey && normalizeUrlKey(entry.href) === locKey)
                hasSelfReference = true;
            if (entry.hreflang) {
                const normalized = entry.hreflang.toLowerCase();
                if (seen.has(normalized))
                    this.#add(context, "GOOGLE_HREFLANG_DUPLICATE", "Each url entry should not repeat the same hreflang value.", entry.path, HREFLANG_SPEC);
                seen.add(normalized);
            }
        }
        if (loc && !hasSelfReference)
            this.#add(context, "GOOGLE_HREFLANG_SELF_REFERENCE_MISSING", "Each url entry with hreflang annotations must include an alternate link for its own loc URL.", path, HREFLANG_SPEC);
    ...
    #openPageMap(element, node, parent, context) {
        if (element.local === "PageMap" && this.#urlActive)
            this.#currentPageMap = { path: element.path, seenChildren: new Set(), lastChildOrder: -1 };
        if (!KNOWN_PAGEMAP_TAGS.has(element.local))
            this.#add(context, "GOOGLE_PAGEMAP_UNKNOWN_TAG", `pagemap:${element.local} is not a recognized Google PageMap sitemap tag.`, element.path, PAGEMAP_XSD, "warning");
        const required = requiredPageMapAttribute(element.local);
        if (required && !getAttribute(node.attributes, required))
            this.#add(context, "GOOGLE_PAGEMAP_REQUIRED_ATTRIBUTE", `pagemap:${element.local} must include @${required}.`, `${element.path}/@${required}`, PAGEMAP_XSD);
        ...
        if (pageMap && element.local !== "PageMap" && parent?.uri === PAGEMAP_NS && parent.local === "PageMap") {
            ...
            if (element.local === "Template") {
                if (pageMap.seenChildren.has(element.local))
                    this.#add(context, "GOOGLE_PAGEMAP_ELEMENT_DUPLICATE", "pagemap:Template can appear only once in pagemap:PageMap.", element.path, PAGEMAP_XSD);
                pageMap.seenChildren.add(element.local);
            }
        }
        if (element.local === "Attribute" && pageMap)
            this.#currentPageMapAttribute = { path: element.path, value: getAttribute(node.attributes, "value") };
    }
    #closePageMap(element, text, context) {
        if (element.local === "Attribute" && this.#currentPageMapAttribute) {
            ...
            if (hasText === hasValue)
                this.#add(context, "GOOGLE_PAGEMAP_ATTRIBUTE_VALUE_INVALID", "pagemap:Attribute must include either text content or @value, but not both.", this.#currentPageMapAttribute.path, PAGEMAP_XSD);
            this.#currentPageMapAttribute = undefined;
        }
        if (element.local === "PageMap")
            this.#currentPageMap = undefined;
    }
    #validateAttributes(element, node, context) {
        for (const attribute of Object.values(node.attributes)) {
            ...
            let code = "";
            if (element.uri === IMAGE_NS)
                code = "GOOGLE_IMAGE_ATTRIBUTE_UNEXPECTED";
            if (element.uri === NEWS_NS)
                code = "GOOGLE_NEWS_ATTRIBUTE_UNEXPECTED";
            if (element.uri === VIDEO_NS) {
                code = "GOOGLE_VIDEO_ATTRIBUTE_UNEXPECTED";
                allowed = isAllowedVideoAttribute(element.local, attribute.local);
            }
            if (element.uri === PAGEMAP_NS) {
                code = "GOOGLE_PAGEMAP_ATTRIBUTE_UNEXPECTED";
                allowed = isAllowedPageMapAttribute(element.local, attribute.local);
            }
            if (element.uri === XHTML_NS && element.local === "link") {
                code = "GOOGLE_HREFLANG_ATTRIBUTE_UNEXPECTED";
                allowed = isAllowedHreflangAttribute(attribute.local);
            }
            if (code && !allowed)
                this.#add(context, code, `Attribute ${attribute.name} is not allowed on this sitemap element.`, `${element.path}/@${attribute.name}`);
        }
    }
    #validateOrder(entry, orderMap, code, element, context) {
        const order = orderMap.get(element.local);
        if (order === undefined)
            return;
        if (order < entry.lastChildOrder)
            this.#add(context, code, `${element.local} appears outside the extension schema order.`, element.path);
        if (order > entry.lastChildOrder)
            entry.lastChildOrder = order;
    }
    #validateDate(context, value, path, code, message, spec, requireTimeSeconds = false) {
        if (!isValidCompleteW3cDateOrDateTime(value, { requireTimeSeconds }))
            this.#add(context, code, message, path, spec);
    }
    #add(context, code, message, path, spec, severity = "error") {
        context.addDiagnostic({ code, severity, source: "google", message, location: context.location(path), spec });
    }
}
export function isExtensionNamespace(uri) {
    return uri === IMAGE_NS || uri === NEWS_NS || uri === VIDEO_NS || uri === PAGEMAP_NS || uri === XHTML_NS;
}
export function shouldCollectExtensionText(element) {
    if (element.uri === IMAGE_NS)
        return element.local === "loc" || element.local === "license";
    if (element.uri === NEWS_NS)
        return new Set(["name", "language", "access", "genres", "publication_date", "title", "keywords", "stock_tickers"]).has(element.local);
    if (element.uri === VIDEO_NS)
        return new Set([
...
function isAllowedVideoAttribute(element, attribute) {
    if (element === "player_loc")
        return attribute === "allow_embed" || attribute === "autoplay";
    if (element === "restriction" || element === "platform")
        return attribute === "relationship";
    if (element === "uploader")
        return attribute === "info";
    if (element === "gallery_loc")
        return attribute === "title";
    if (element === "content_segment_loc")
        return attribute === "duration";
    if (element === "id")
        return attribute === "type";
    if (element === "price")
        return attribute === "currency" || attribute === "type" || attribute === "resolution";
    return false;
}
function isAllowedPageMapAttribute(element, attribute) {
    if (element === "Template")
        return attribute === "src";
    if (element === "DataObject")
        return attribute === "type" || attribute === "id";
    if (element === "Attribute")
        return attribute === "name" || attribute === "value";
    return false;
}
function isAllowedHreflangAttribute(attribute) {
    return attribute === "rel" || attribute === "hreflang" || attribute === "href";
}
function requiredPageMapAttribute(element) {
    if (element === "Template")
        return "src";
    if (element === "DataObject")
        return "type";
