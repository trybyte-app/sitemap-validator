...
export async function openLiveUrlDataset(path, options = {}) {
...
export async function createLiveUrlDatasetCollector(options) {
...
function createCountingCollector() {
    let totalUrls = 0;
    let settled = false;
    return {
        async add() {
...
function createDataset(path, format, totalUrls, cleanup = async () => { }) {
    let closed = false;
    return {
        totalUrls,
...
async function detectDatasetFormat(path) {
...
async function countRecords(path, format, onProgress) {
    let count = 0;
...
async function* iterateUrlsFileAsRecords(path) {
...
async function* iterateLines(path) {
